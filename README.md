...
```bash
conda create -n job_search_env python=3.9 -y
conda activate job_search_env
pip install -e .
```

2. **Environment variables (optional)**

Create a `.env` (or export vars) if you want extra functionality:

```
# Database – defaults to fallback SQLite
...
The tests create a temporary SQLite database in `./data/jobsearch_test.db` and run a full end-to-end health-check.

## Project Structure

```
src/
  api_clients/     # external job board APIs
  scrapers/        # GitHub, InternList, ATS site scrapers
  database/        # SQLAlchemy models & helpers
  web_app/         # Flask routes (+ templates/ static/ )
  notifications/   # Email & SMS notifiers
  ranking/         # Job scoring & ranking engine
```

## Common Issues

* **PostgreSQL not running** – The app will automatically fall back to SQLite.
* **Missing SMTP/Twilio creds** – Notification modules will log a warning and continue.
* **Template errors** – Ensure `src/web_app/templates` exists (already in repo) and contains the HTML files.
