...
class WorkdayATSScraper(BaseScraper):
    ...
    # Backwards-compat: simple wrapper to required abstract method
...
class ATSScraperFactory:
    ...
    def create_scraper(ats_type: ATSType, config_obj=None, **kwargs) -> BaseScraper:
        ...
        if scraper_class:
            return scraper_class(config=config_obj, **kwargs)
        raise ValueError(f"No scraper found for ATS type: {ats_type}")
