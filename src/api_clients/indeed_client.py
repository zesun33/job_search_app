...
class JobAPIManager:
    """
    Manager class to coordinate multiple job board APIs
    Provides unified interface for searching across multiple sources
    """
    ...
    def _initialize_apis(self):
        ...
        # Initialize Indeed API if key is available
        if config.INDEED_API_KEY:
            self.apis['indeed'] = IndeedAPI(config.INDEED_API_KEY)
        
        # Initialize ZipRecruiter API if key is available
        if config.ZIPRECRUITER_API_KEY:
            self.apis['ziprecruiter'] = ZipRecruiterAPI(config.ZIPRECRUITER_API_KEY)
        
        # Add other APIs as they become available
        
        logger.info(f"Initialized {len(self.apis)} job board APIs: {list(self.apis.keys())}")
    ...
    def search_all_sources(self, 
        ...
        """
        Search all available job board APIs
        ...
        for source_name, api in self.apis.items():
            try:
                logger.info(f"Searching {source_name} for: {keywords}")
                ...
                logger.info(f"Retrieved {len(jobs)} jobs from {source_name}")
    ...
    def get_source_stats(self) -> Dict[str, Dict]:
        """Get statistics for all API sources"""
        stats = {}
        for source_name, api in self.apis.items():
            stats[source_name] = api.get_stats()
        return stats 