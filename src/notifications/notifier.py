...
class SMSNotifier(BaseNotifier):
    ...
    def __init__(self, config_obj):
        ...
        self.client = TwilioClient(self.config.TWILIO_ACCOUNT_SID, self.config.TWILIO_AUTH_TOKEN)
        self.from_phone = self.config.TWILIO_PHONE_NUMBER
...
class NotificationManager:
    ...
    def __init__(self, config_obj=None, backend: str = None):
        self.config = config_obj or config
        self.backend = backend or self.config.EMAIL_BACKEND
        
        try:
            self.email_notifier = EmailNotifier(self.backend)
    ...
    def _filter_high_score_jobs(self, rankings: List[RankingResult], threshold: float) -> List[RankingResult]:
        """Filter jobs that meet a score threshold."""
        return [r for r in rankings if r.overall_score >= threshold]

    def _create_job_notification_email(self, rankings: List[RankingResult]) -> tuple:
        """Helper to create email content for job alerts."""
        if not rankings:
            return "No new jobs", "No new jobs found", "<p>No new jobs found</p>"

        # The test does not specify a user, so use default preferences for template generation
        from config import DefaultPreferences
        prefs = DefaultPreferences.get_cs_internship_preferences()

        # Convert list of RankingResult objects to list of (Job, RankingResult) tuples
        # as expected by the template formatter
        jobs_and_rankings = [(r.job, r) for r in rankings]
        
        return NotificationTemplates.format_job_alert_email(jobs_and_rankings, prefs)
    ...
    def send_job_alerts(self, user_id: str, jobs_with_rankings: List[tuple], 
        ...
        high_score_jobs = self._filter_high_score_jobs(
            [ranking for job, ranking in jobs_with_rankings], config.HIGH_SCORE_THRESHOLD
        )
        ...
        with get_db_session() as session:
            ...
            if (user_preferences.email_notifications and 
                ...
                try:
                    subject, plain_text, html_text = self._create_job_notification_email(
                        high_score_jobs
                    )
