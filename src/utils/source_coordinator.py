...
from ..scrapers.ats_scrapers import ATSScraperFactory
...
class JobSourceCoordinator:
            ...
            for term in search_terms:
                try:
                    jobs = await self.api_manager.search_all_sources(
                        ...
                        job_type="",
                        limit_per_source=max_jobs // len(search_terms) if max_jobs else 100
                    )
    ...
    async def _get_company_scraper(self, company_config) -> Optional[BaseScraper]:
        ...
        if company_config.ats_type:
            return ATSScraperFactory.create_scraper(ats_type=company_config.ats_type, config_obj=self.config)
        return None
