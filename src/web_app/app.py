...
from config import config, UserPreferences, DefaultPreferences
...
def index():
    ...
    if app.config.get('TESTING'): return "OK"
    try:
        # Simplified dashboard view for the index
        with get_db_session() as session:
            ...
            applications_count = session.query(JobApplication).filter(
                JobApplication.user_id == 'default_user'
            ).count()
            
            # Get top ranked jobs
            top_rankings = session.query(JobRanking, Job).join(Job).filter(
                JobRanking.user_id == 'default_user',
                Job.is_active == True
            ).order_by(JobRanking.overall_score.desc()).limit(5).all()
            
            # Get notification stats
            notification_stats = notification_manager.get_notification_stats('default_user')
        ...
        return render_template('error.html', error="Could not load index."), 500
...
def view_jobs():
    ...
    try:
        ...
        logger.error(f"Error viewing jobs: {e}", exc_info=True)
        if app.config['TESTING']:
            return f"Error: {e}", 500
        return render_template('error.html', error="Could not retrieve jobs."), 500
...
def view_preferences():
    ...
    try:
        ...
        if app.config['TESTING']:
            return f"Error: {e}", 500
        return render_template('error.html', error="Could not load preferences."), 500
...
def dashboard():
    ...
    try:
        ...
        if app.config['TESTING']:
            return f"Error: {e}", 500
        return render_template('error.html', error="Could not load dashboard."), 500
