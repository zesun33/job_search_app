...
from config import Config, DefaultPreferences
from src.database.models import JobData, UserProfile, Session, create_tables
from src.database import get_session, init_database
from src.ranking.job_ranker import JobRanker, RankingExplanation
...
from src.scrapers.intern_list_scraper import InternListScraper
from src.scrapers.ats_scrapers import ATSScraperFactory, ATSType
...
class SystemTester:
    ...
    async def test_scrapers(self):
        ...
        try:
            logger.info("Testing GitHubScraper init...")
            github_scraper = GitHubScraper(self.config)
            logger.info("GitHubScraper OK.")
            github_status = github_scraper.get_repository_status()
            self.test_results["scrapers"]["github"] = {"success": len(github_status) > 0, "repositories_configured": len(github_status), "details": github_status}

            logger.info("Testing InternListScraper init...")
            intern_scraper = InternListScraper(self.config)
            logger.info("InternListScraper OK.")
            intern_status = await intern_scraper.test_scraping()
            self.test_results["scrapers"]["intern_list"] = {"success": intern_status.get("accessible", False), "details": intern_status}

            logger.info("Testing ATSScraperFactory init...")
            supported_ats = ATSScraperFactory.get_supported_ats_types()
            ats_results = {}
            for ats_type in supported_ats:
                logger.info(f"Creating ATS scraper for {ats_type.value}")
                scraper = ATSScraperFactory.create_scraper(ats_type=ats_type, config_obj=self.config)
                logger.info(f"ATS scraper for {ats_type.value} OK.")
                ats_results[ats_type.value] = {"success": True, "message": f"{ats_type.value} scraper created successfully"}
            self.test_results["scrapers"]["ats"] = ats_results
            logger.info("ATSScraperFactory OK.")

            logger.info("Testing JobAPIManager init...")
            api_manager = JobAPIManager(self.config)
            logger.info("JobAPIManager OK.")
            api_status = await api_manager.test_connections()
            self.test_results["scrapers"]["api_clients"] = {"success": any(client.get("available", False) for client in api_status.values()), "details": api_status}

            logger.info("Testing JobSourceCoordinator init...")
            coordinator = JobSourceCoordinator(self.config)
            logger.info("JobSourceCoordinator OK.")
            source_status = await coordinator.get_source_status()
            self.test_results["scrapers"]["coordinator"] = {"success": "error" not in source_status, "details": source_status}

        except Exception as e:
            logger.error(f"Scraper tests failed: {str(e)}", exc_info=True)
    ...
    async def test_notifications(self):
        ...
        try:
            ...
            email_subject, email_body, _ = notifier._create_job_notification_email([ranking])
